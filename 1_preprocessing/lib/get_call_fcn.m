...
for iC = 1:proc_call_num
  % Load params
  tolerance = data.param.tolerance;  % tolerance for finding calls around peak of max channel, 2ms?
  ...
  call_short_taper(isnan(call_short_taper)) = 0;
  ...
  elseif strcmp(data.param.PSD_type,'pwelch')
    if length(call_short_taper) > 120 %128
      call_psd=pwelch(call_short_taper,120,110,call_freq_vec,data.mic_data.fs);
      call_psd_dB = 10*log10(call_psd);
    else
