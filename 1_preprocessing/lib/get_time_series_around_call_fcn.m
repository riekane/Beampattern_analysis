...
extract_len_pt = data.param.extract_call_len_pt;  % sample points, 10 ms
...
call_mtx_simple = data.mic_data.sig(data.mic_data.call(curr_call_global_idx).locs + extract_len_idx,:);
...
current_call_detect_idx = max_pk_idx_in_ch(max_ch_idx) + data.mic_data.call(curr_call_global_idx).locs + extract_len_idx(1) -1;
...
[~,call_emission_idx_in_traj] = min(abs(current_call_detect_time - data.time_of_call_at_mic(:,max_ch_idx)));
